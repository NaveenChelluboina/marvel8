...
export class AssetTypeComponent implements OnInit {
  ...
  public popoverMessage: string = 'Are you sure you want to delete this asset?';
  ...
  constructor(public appSettings: AppSettings,public alertService:AlertService,
    ...
    ngOnInit() {
      this.tableList = [];
      this.getAssetTypes({});
      let userdata = JSON.parse(sessionStorage.getItem('trs_user_info')).user_permissions[11];
    ...
    filterSearch() {
      ...
      if(this.userControl.value) {
        filterObj['asset_name'] = this.userControl.value.trim();
      }
    ...
    AssetNameClicked(order) {
      if(order) {
        this.assets.sort(function(a, b) {
          var titleA = a.asset_name, titleB = b.asset_name;
        ...
        this.assets.sort(function(a, b) {
          var titleA = b.asset_name, titleB = a.asset_name;
          if (titleA < titleB) return -1;
          if (titleA > titleB) return 1;
          return 0;
        });
    ...
    public getAssetTypes(filter) {
      filter['per_page'] = this.pageSize;
      filter['page'] = this.currentPage;
      this.carrierService.getAssetTypes(filter).then(data => {
        if(data.success) {
          for(let i = 0 ; i < data.results.length ; i++) {
            if(data.results[i].asset_type == 1) {
              data.results[i].asset_type_name = "Power Unit"
            }
            if(data.results[i].asset_type == 2) {
              data.results[i].asset_type_name = "Trailer"
            }
          }
    ...
    public openAssetDialog(id) {
      let dialogRef = this.dialog.open(AddAssetTypeComponent, {
        data: id,
        height: 'auto',
        width: '450px',
        autoFocus:false
      ...
      dialogRef.afterClosed().subscribe(data => {
        if(data == 'save') {
          if(this.usingObject)
          this.getAssetTypes(this.usingObject);
          else 
          this.getAssetTypes({});
        }
      });
    ...
    openDeleteDialog(data) {
      ...
      dialogRef.afterClosed().subscribe(data => {
        if (data != null && data !== undefined) {
          let finalObj = {};
          finalObj['asset_type_id'] = data.asset_type_id;
          finalObj['is_deleted'] = 1;
          this.carrierService.updateAssetType(finalObj).then(data => {
            if (data.success) {
              this.alertService.createAlert("Asset-Type deleted successfully", 1);
              if(this.usingObject)
                 this.getAssetTypes(this.usingObject);
                 else 
                this.getAssetTypes({});
    ...
    updateAssets(id,status,value) {
      let finalObj = {};
      finalObj['asset_type_id'] = id;
      if(value == 'delete') {
      ...
      this.carrierService.updateAssetType(finalObj).then(data => {
        if(data.success) {
          if(value == 'active')
          this.alertService.createAlert("Asset-Type " + (status ? 'activated' : 'deactivated') + ' successfully',1);
          if(value == 'delete')
          this.alertService.createAlert("Asset-Type deleted successfully",1);
          let filterObj = {};
          if(this.status_filter) {
          ...
          if(this.userControl.value) {
            filterObj['asset_name'] = this.userControl.value;
          } 
          this.getAssetTypes(filterObj);
        }
        else {
